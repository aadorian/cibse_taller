...
La estructura del documento es la siguiente. En la Sección \ref{sec:intro} se presenta la introducción de conceptos de data profiling, modelo de calidad de datos y dimensiones de calidad. 
En la sección \label{sec:reporte}
...
\cite{abedjan2017data}. La elaboración de ``data profiling'' es el conjunto de actividades y procesos para determinar los metadatos sobre un conjunto de datos determinado. Entre los resultados mas frecuentes se encuentran el establecer estadísticas por columna, como la cantidad de valores nulos y valores distintos en una columna, su tipo de datos o los patrones más frecuentes de sus valores de datos. Los metadatos que son más difíciles de calcular involucran múltiples columnas, como dependencias de inclusión, funcionales y de orden \cite{abedjan2017data}.
El proceso de descubrimiento de metadatos se conoce como ``data profiling''. Las actividades de creación de perfiles van desde enfoques ``ad-hoc'', como subconjuntos aleatorios de datos o la formulación de consultas de agregación, hasta la inferencia sistemática de metadatos a través de algoritmos de creación de perfiles \cite{abedjan2018introduction}. La creación de ``data profiling'' juega un papel importante en distintos casos de uso, como ser la exploración de datos, la integración de datos y el análisis de datos \cite{abedjan2018introduction}. El objetivo de la elaboración de``data profiling''  mide la consistencia y precisión de los datos, detecta duplicaciones de los mismos para obtener el valor correcto de los datos que podrían usarse en la toma de decisiones \cite{elbaghazaoui2021data}.
...
El modelo de calidad de datos  ISO/IEC 25012 esta compuesto de quince características dentro de las cuales destacamos cuatro : Exactitud, Completitud , Consistencia y Comprensibilidad. \url{https://iso25000.com/index.php/en/iso-25000-standards/iso-25012}.
...
Se evaluaran la dimensión de \textbf{Exactitud} ) y  Unicidad. 
...
El método de medición será calculado en forma directa midiéndolo directamente
...
En la Tabla~\ref{tab:modelometricainstanciada} se presentan las métricas instanciadas. Las metricas M1, M2, M3 y M4 fueron instanciadas sobre el dataset de Operadores. La métrica M1, refería a el mail, bew y telefono de los cuales se verifico la correctitud del formato de mail estandar. Las otras métricas instanciadas referian a tipo de operador, departamente, longigud y latitud. 
\begin{table*}
...
Las medidas de calidad se almacenarán en  Google Drive y las distintas versiones de reportarán en el repositorio de GIT \footnote{\url{https://github.com/aadorian/cibse_taller}}. 
...
La evaluación final de la calidad se presenta en detalle la Tabla \ref{tab:evaluacionfinal1} y \ref{tab:evaluacionfinal1} respectivamente. 
...
El nivel de confianza en la calidad de datos estudiada es de bajo nivel en general. Un análisis mas detallado y ejecución de un diseño mas elaborado permitiría identificar en mejor detalle la calidad de los mismos. 
